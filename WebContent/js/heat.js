...
var PrevSelectedArr;// array hold previous selectedArr for little Dynamic
					// Programming its not really Dynamic Programming
var newSelected = [];// array contine the new selected rec's wich it is
						// selectedArr-PrevSelectedArr if updateSelect=1
var updateSelect = 0;// bool var if we only need to update the selected his
...
var MAX_MAG;
var AllMaxMag;
var max_normto = 1900;// a define var that contine the the value of the
 ...
 * ################################### @author ${Ebraheem_kashkoush}
 * ################### ###################################selector on Change+
 * update DataArray to selected region ###################
 * ################################### tags: #select ###################
 * ####################################################################################################################
 * 
 */
...
function selectorChange(Limits) {
	...
	// Rounding up containing Rectangle (to biger Rectangle containing full
	// Rectangles)
	NewCord = {
		N : -1,
		S : -1,
	...
	for (var i = iFROM; i < iTO; i++)
		// need a check tomorrow
		for (var j = jFROM; j < jTO; j++)
			selectedArr.push(i * lon_div + j);

	// check if to update or calc the all
	/*
	 * not completed so it have many errors i see that for now we not really
	 * need it becouse it calc the heat very fast console.log("PrevSelectedArr ="
	 * +PrevSelectedArr) console.log("selectedArr =" +selectedArr) var ko=0;
	 * updateSelect=1; var coco=0; for(var ki=0;ki<PrevSelectedArr.length;ki++){
	 * while((PrevSelectedArr[ki]!=selectedArr[ko])&&(PrevSelectedArr[ki]>selectedArr[ko])&&(ko<selectedArr.length)){
	 * newSelected.push(selectedArr[ko]); ko++; } if(ko>(selectedArr.length-1)){
	 * updateSelect=0; } coco=1; } if(coco){ while(ko<selectedArr.length){
	 * newSelected.push(selectedArr[ko]); ko++; } coco=0; }
	 */
		...
		for (var i = 0; i < selectedArr.length; i++) {
			...
			if (RegionArr[selectedArr[i]].length > 1) {
				for (var j = 1; j < RegionArr[selectedArr[i]].length; j++) {
					...
					// hint for improve we need only this check on the border so
					// to get little improve you cans split it to to for's
					if ((tempLat >= Limits.S) && (tempLat <= Limits.N)
							&& (tempLon >= Limits.W) && (tempLon <= Limits.E)) {
...
function turnselectorOff() {
	...
	document.getElementById("NoOfEarthquake").innerHTML = " : "
			+ DataArray.length;
 ...
 * #############################################################################################################################
 * ################################### @author ${Ebraheem_kashkoush}
 * ################### ###################################initialize RegionArr +
 * check where the point on the region array ###################
 * ################################### tags: #RegionArr ###################
 * ############################################################################################################################
 * 
 */
	...
	for (var i = 0; i < lat_div; i++) {
		for (var j = 0; j < lon_div; j++) {
			...
			if ((j < lon_div - 1) && (i < lat_div - 1))
				RegionArr[i * lon_div + j].push({
					E : (limits.W + (j + 1) * lon_unit),
					W : (limits.W + j * lon_unit),
					N : limits.S + (i + 1) * lat_unit,
					S : limits.S + i * lat_unit
				});
			else {
 ...
 * ################################### @author ${Ebraheem_kashkoush}
 * ################### ###################################calc magnitude to
 * correct one that show the power of earthquake ###################
 * ################################### tags: #mag ###################
 * ############################################################################################################################
 * 
 */
 ...
 * ################################### @author ${Ebraheem_kashkoush}
 * ################### ###################################DATA init region
 * array+ data array+ localstorge +update ###################
 * ################################### tags: #DATA #region ###################
 * ############################################################################################################################
 * 
 */
...
function Data(key, x) {
	...
	// ////////////////////////////////////////////////////////////////////////////////////
	/*
	 * DataArray STRUCTER ARRAY[ARRAY]=features [quake.properties]
	 * 
...
function CirclesOFF() {
	...
	circVis = 0;
	map.removeLayer(areaSelect);
	// areaSelect.remove(); we cant remove becouse mybe he chose select after
	// that
	on_select = 0;
...
function CirclesON() {
	...
	if (!circVis) {
		...
		selectWh(w, h);
		// getPanes() : Returns an object with different map panes (to render
		// overlays in)
		// overlayPane() : Pane for overlays like polylines and polygons.
		var svg = d3.select(map.getPanes().overlayPane).append("svg"),
		// The SVG <g> element is used to group SVG shapes together. Once
		// grouped you can transform the whole
		// group of shapes as if it was a single shape. This is an advantage
		// compared to a nested <svg> element
		// which cannot be the target of transformation by itself

		// leaflet-zoom-hide : class to the DOM elements you create for the
		// layer so that it hides during zoom animation.
		// Implementing zoom animation for custom layers is a complex topic and
		// will be documented separately in future,
		// but meanwhile you can take a look at how it's done for Leaflet layers
		// (e.g. ImageOverlay) in the source.
		g = svg.append("g").attr("class", "leaflet-zoom-hide"), color = d3.scale
				.log()
				// max sig value is 1000 the the value interpolate bettwen this
				// color
				.domain([ 166, 266, 566, 1000 ]).range(
						[ "#377eb8", "#4daf4a", "#e41a1c" ])// color range
				.interpolate(d3.interpolateHcl);// interpolate it wiht HCL color
												// not LAB

		d3.json(" ",
				function(collection) {
					var path = d3.geo.path().projection(project), feature = g
							.selectAll("path").data(data2.features).enter()
					...
					// viewreset : Fired when the map needs to redraw its
					// content (this usually happens on map zoom or load). Very
					// useful for creating custom overlays.
					map.on('viewreset', reset);
					reset();

					function reset() {
						var Ds = project([ bounds.getSouthWest().lng,
								bounds.getSouthWest().lat ])[1], Dw = project([
								bounds.getSouthWest().lng,
								bounds.getSouthWest().lat ])[0], Dn = project([
								bounds.getNorthEast().lng,
								bounds.getNorthEast().lat ])[1], De = project([
								bounds.getNorthEast().lng,
								bounds.getNorthEast().lat ])[0];

						g.attr("transform", "translate(" + -1 * (Dw) + "," + -1
								* (Dn) + ")");

						svg.attr("height", Ds - Dn).attr("width", De - Dw)
						// we need to margin because in zooming the bounds is
						// change and we need to marg them
						.style("margin-left", Dw + "px").style("margin-top",
								Dn + "px");

						feature.attr('d', path).attr("fill-opacity", 0.6).on(
								"mouseover",
								function() {
					...
					function get_Sig(d) {
						// sig: A number describing how significant the event
						// is. Larger numbers indicate a more significant event.
						// This value is determined on a number of factors,
						// including: magnitude, maximum MMI, felt reports, and
						// estimated impact.
...
function REINIT(newdata) {
	// for external re INIT before to use it should give a new data to data var
	// data=NEW_JSON_DATA
...
function initMap() {
	...
	/*
	 * map = new L.Map('map', { center: bounds.getCenter(), zoom: 5, layers:
	 * [osm], maxBounds: bounds, maxBoundsViscosity: 1.0 });
...
function REinitEK(x) {
	// like INIT but we not re define map and also to intilaize dar var from
	// local storge or file we should send 1 else 0
	lastclick = 0;
	MAX_MAG = 0;
