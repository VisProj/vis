...
/* set the parallel diagram element*/
...
function datamining(data)
{
	...
	for(i=0;i<data_length; i++)
	{
		try
		{
			...
			depth = parseInt(data[i].geometry.coordinates[2]);
...
{
	/*
	 * if two quakes in the data have the same location , then merge them into
	 * one ;
	 * 
	 */
	var i ; 
	...
	/*
	 * Get a String with all the diffrent locations of the earthquakes in the
	 * data of the format "location1,location2,...,locationN" initialize a data
	 * Arrya with all locations .
	 */
	for(i=0;i<data_length;i++)
	{
	...
	//alert(result.length);
	/*
	 * due to the high number of Diffrent locations , we want to join All the
	 * Locations that have <= number_of_quakes_thershold  , we
	 * refer to the joined element by "Others " ( which is the first element of
	 * result array
	 */
	...
	for(i=1;i<dyn_length; i++)
	{
		...
		if(result[i].number <=number_of_quakes_thershold )
	...
	// Set Other locations variable in connectors.js
	...
	for(i=0;i<results_length;i++)
	{
		...
		count+=norm;
		result[i].mag =magnFactor * result[i].mag/norm; 
		// if(isNaN(result[i].mag)) //alert("gere");
		result[i].depth = parseInt(result[i].depth/norm) ; 
...
function GetAccuracy(earthquake)
{
	...
	/*
	 * nst : [1,200] the total number of seismic stations used to determine
	 * earthquake location (the higher te better)
	 */
	...
	/*
	 * gap : [0.0,180.0] the largest azimuthal gap between azimuthally adjacent
	 * stations (in degrees). the smaller this number , the more reliable is the
	 * calculated horizontatl position of the earthquake .
	 */
   ...
   /*
	 * dmin : Horizantal distance from the epicenter of the nearest station (in
	 * degree ) , 1edgreee ~ = 111.2 killomiters the smaller this number the
	 * more reliable is the calculated depth . range [0.4,7.1]
	 */
	var dmin = earthquake.dmin; 
	dmin = (7.1-dmin)/6.7 *10 ; // (7.1-dmin)/(7.1-0.4)
	
	/*
	 * horizontalError: [0,100] uncertainty of reported location of the event in
	 * kilometers
	 */
	...
	/*
	 * depthError : [0,100] uncertainty of reported depth of the event in
	 * kilometers
	 */
	var depthError = earthquake.depthError;
	depthError =(100-depthError)*0.2;
	/*
	 * magError : [0,100] uncertainty of reported magnitude of the event
	 */
...
{
	// location is a string of the foramt : [ distans in kilometers
	// (North/South/west.east) of "landmark " , city ]
